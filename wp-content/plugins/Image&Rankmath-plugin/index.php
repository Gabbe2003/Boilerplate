<?php
/**
 * Plugin Name:     Pixabay Image Connector (No Cron)
 * Description:     Exposes Rank Math SEO fields via REST, swaps featured images via Pixabay or a fallback, and provides settings, manual sync, batch size, and daily logs under Tools → Pixabay Sync. (Cron removed.)
 * Version:         1.11.0
 * Author:          You
 */
...
/* ------------------------------------------------------------------
 * 2. Default settings (cron keys removed)
 * ---------------------------------------------------------------- */
...
function ypseo_get($key)
{
    ...
    return (false === $opt) ? (ypseo_defaults()[$key] ?? null) : $opt;
}
...
/* ------------------------------------------------------------------
 * 4. (Removed) Cron scheduling
 * ---------------------------------------------------------------- */
// All cron-related hooks and handlers removed.
...
{
    $ttl = DAY_IN_SECONDS * (int) ypseo_get('cache_days');
    $batch = (int) ypseo_get('batch_size');
    $posts = array_slice($posts, 0, max(1, $batch));
    ...
    foreach ($posts as $post) {
        ...
        $ok = ypseo_set_post_thumbnail_from_url(
            $post,
            $img,
            $kw ?: $post->post_title,
...
/* ------------------------------------------------------------------
 * 7. (Removed) Cron handler
 * ---------------------------------------------------------------- */
// Removed. Use manual sync below.
...
function ypseo_admin_page()
{
    ...
    // Handle saving settings (cron options removed)
    ...
    if (isset($_POST['sync_submit']) && check_admin_referer('ypseo_sync')) {
        ...
        // Save a daily log bucket for reference
        $date_bucket = gmdate('Y-m-d');
        ypseo_add_log($date_bucket, $log_entries);
    }
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td><input type="date" name="to"
                            value="<?php echo esc_attr($_POST['to'] ?? gmdate('Y-m-d', strtotime('-1 day'))); ?>" />
                    </td>
                </tr>
                <tr>
